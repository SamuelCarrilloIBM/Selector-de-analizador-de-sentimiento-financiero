"""
Test NVIDIA: microsoft/deberta-v3-small — zero-shot sobre nvidia_testing.csv
...
# id2label: {0: "bearish", 1: "neutral", 2: "bullish"} → bearish=negative, bullish=positive
DEBERTA_IDX_TO_INTERNAL = {0: 0, 1: 1, 2: 2}  # bearish→negative(0), neutral→1, bullish→positive(2)
...
for s in samples:
    ...
    pred = DEBERTA_IDX_TO_INTERNAL[int(torch.argmax(logits, dim=-1).item())]
...
csv_out = f"{RESULTS_DIR}/deberta_nvidia_predictions.csv"
...
json_out = f"{RESULTS_DIR}/deberta_nvidia_metadata.json"
meta = {
    "model": MODEL_NAME, "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
    "dataset": CSV_PATH, "n_test": len(true_labels),
    "approach": "zero-shot (fine-tuned on financial text classification, bearish/bullish→neg/pos)",
    "performance": {
