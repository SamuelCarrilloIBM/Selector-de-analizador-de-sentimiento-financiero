"""
Test individual: microsoft/deberta-v3-small
...
# DeBERTa v3 tiene problemas de NaN en MPS — forzar CPU
...
RESULTS_DIR = "../../results/bert/deberta"
LABEL_MAP   = {"negative": 0, "neutral": 1, "positive": 2}
LABEL_NAMES = ["negative", "neutral", "positive"]
# DeBERTa v3 tiene problemas de gradientes en MPS — forzar CPU
os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"

def compute_metrics(eval_pred):
    logits, labels = eval_pred
    # Guard contra NaN/Inf en logits (puede ocurrir en primeras iteraciones de DeBERTa)
    ...
    p_cls, r_cls, f1_cls, _ = precision_recall_fscore_support(labels, preds, average=None, labels=[0,1,2], zero_division=0)
    return {
...
def export_results(model_name, headlines, true_labels, pred_labels, metrics,
    ...
    meta = {
        ...
        "approach": "fine-tuning (3 epochs, lr=2e-5, batch=8)",
        "performance": {
...
ds_small = ds["train"].select(range(200))  # reducido para CPU (DeBERTa no soporta MPS)
...
# Forzar CPU — DeBERTa v3 produce NaN en MPS por ops no soportadas
model = model.to("cpu")
import torch
_orig_is_available = torch.backends.mps.is_available
torch.backends.mps.is_available = lambda: False
args = TrainingArguments(
    output_dir=f"{RESULTS_DIR}/checkpoints_deberta",
    eval_strategy="epoch", save_strategy="no",
    learning_rate=1e-5, per_device_train_batch_size=8,
    per_device_eval_batch_size=8, num_train_epochs=3,
    weight_decay=0.01, logging_steps=100, report_to="none",
...
export_results(MODEL_NAME, headlines, true_labels, pred_labels,
               metrics, train_time, inf_times, mem_mb, RESULTS_DIR, "deberta")
