"""
Test NVIDIA: distilroberta-base — zero-shot sobre nvidia_testing.csv
...
# id2label: {0: "negative", 1: "neutral", 2: "positive"} — coincide con LABEL_NAMES, sin remapeo
...
csv_out = f"{RESULTS_DIR}/distilroberta_nvidia_predictions.csv"
...
json_out = f"{RESULTS_DIR}/distilroberta_nvidia_metadata.json"
meta = {
    "model": MODEL_NAME, "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
    "dataset": CSV_PATH, "n_test": len(true_labels),
    "approach": "zero-shot (fine-tuned on financial news sentiment, NVIDIA dataset)",
    "performance": {
