"""
Test NVIDIA: bert-base-uncased — zero-shot sobre nvidia_testing.csv
...
FINBERT_IDX_TO_INTERNAL = {0: 2, 1: 0, 2: 1}  # positive→2, negative→0, neutral→1
...
if __name__ == "__main__":
    ...
    for s in samples:
        ...
        pred = FINBERT_IDX_TO_INTERNAL[pred]
    ...
    csv_out = f"{RESULTS_DIR}/bert_nvidia_predictions.csv"
    ...
    json_out = f"{RESULTS_DIR}/bert_nvidia_metadata.json"
    meta = {
        "model": MODEL_NAME, "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "dataset": CSV_PATH, "n_test": len(true_labels),
        "approach": "zero-shot (ProsusAI/finbert — fine-tuned on Financial PhraseBank)",
        "performance": {
