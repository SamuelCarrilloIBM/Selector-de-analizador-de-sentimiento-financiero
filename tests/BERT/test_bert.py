"""
Test individual: bert-base-uncased
Fine-tuning + evaluación sobre dataset de sentimiento financiero.
Genera: predictions_csv + metadata_json
"""
# ── IMPORTANTE: estas variables deben estar ANTES de cualquier otro import ──
import os
...
os.environ["GRPC_VERBOSITY"]                      = "ERROR"  # silencia logs gRPC/mutex
os.environ["GLOG_minloglevel"]                    = "3"      # silencia logs C++
os.environ["OBJC_DISABLE_INITIALIZE_FORK_SAFETY"] = "YES"    # fix mutex en macOS

# ── Forzar spawn antes de importar torch/transformers ───────────────────────
...
def export_results(model_name, headlines, true_labels, pred_labels, metrics,
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for i, (h, t, p) in enumerate(zip(headlines, true_labels, pred_labels), 1):
            writer.writerow({"id": i, "noticia": h,
                             "etiqueta_real": LABEL_NAMES[t],
                             "etiqueta_predicha": LABEL_NAMES[p],
                             "correcto": "si" if t == p else "no"})
    ...
    meta = {
        ...
        "approach": "fine-tuning (3 epochs, lr=2e-5)",
        "performance": {
...
# ── Todo el código de ejecución dentro de main guard ────────────────────────
# Necesario en Mac porque usa `spawn` para multiprocessing; sin esto los
# workers del DataLoader re-ejecutan el módulo y causan deadlocks (mutex block).
if __name__ == "__main__":
    ...
    train_ds = split["train"].rename_column("sentiment", "labels")
    test_ds  = split["test"].rename_column("sentiment", "labels")
    train_ds = train_ds.map(lambda b: {"labels": [LABEL_MAP[l] for l in b["labels"]]}, batched=True)
    ...
    args = TrainingArguments(
        output_dir=f"{RESULTS_DIR}/checkpoints_bert",
        eval_strategy="epoch",
        save_strategy="no",
        learning_rate=2e-5,
    ...
    print(f"Train: {train_time:.1f}s | Inferencia: {np.mean(inf_times):.2f}ms/muestra | RAM: {mem_mb:.0f}MB")

    print("\n📁 Exportando resultados...")
    export_results(MODEL_NAME, headlines, true_labels, pred_labels,
                   metrics, train_time, inf_times, mem_mb, RESULTS_DIR, "bert")